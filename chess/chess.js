...
Rook.prototype.isValidMove = function(toSquare,n=1){
        ...
        if(currentPlayer.kingMoved ==false){
            ...
            if(currentPlayer.king.x==5&&currentPlayer.king.y==Y){
                if(currentPlayer.checked==false){
                   if(movementX==2){
                       if(getSquare(8,Y).piece instanceof Rook){
                       ...
                       if(getSquare(1,Y).piece instanceof Rook){
...
let setup = function(){
	...
	pieces.push(new Rook(1, 1, "black"));
	...
	pieces.push(new Rook(8, 1, "black"));
	...
	pieces.push(new Rook(1, 8, "white"));
	...
	pieces.push(new Rook(8, 8, "white"));
	for(let i = 0; i < pieces.length; i++){
...
let promote = function(type){
	...
	switch(type){
		case "queen":
			...
			break;
		case "rook":
			newPiece = new Rook(oldPiece.x, oldPiece.y, oldPiece.color);
			break;
		case "bishop":
